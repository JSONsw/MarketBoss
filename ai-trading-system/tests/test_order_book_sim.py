...
def test_full_fill_buy():
    book = generate_synthetic_book(
        mid_price=100.0, depth=3, tick=0.5, size=50.0
    )
    asks = book["asks"]
...
def test_partial_fill_buy():
    book = generate_synthetic_book(
        mid_price=50.0, depth=2, tick=1.0, size=10.0
    )
    asks = book["asks"]
...
def test_sell_fill_and_avg():
    book = generate_synthetic_book(
        mid_price=200.0, depth=4, tick=0.25, size=20.0
    )
    bids = book["bids"]
    res = simulate_limit_order_fill(
...
def test_market_order_consumes_book():
    book = generate_synthetic_book(
        mid_price=10.0, depth=3, tick=0.1, size=5.0
    )
    asks = book["asks"]
